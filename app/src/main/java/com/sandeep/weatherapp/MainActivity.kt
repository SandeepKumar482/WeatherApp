...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        window.setFlags(ACTION_NEXT_HTML_ELEMENT, ACTION_NEXT_HTML_ELEMENT)
        3328.also { window.decorView.systemUiVisibility = it }
       ...
       weatherAdapter= RVweatherAdapter(this,weatherList)
        ...
        { val perm ="android.Manifest.permission.ACCESS_FINE_LOCATION,android.Manifest.permission.ACCESS_COARSE_LOCATION"
    ...
    private fun getWeatherInfo(cityName:String){
        val url :String=
            "http://api.weatherapi.com/v1/current.json?key=adbcd73789bd473a9a5112234210508&q=$cityName&aqi=yes"
        ...
        val jsonObjectRequest = object :JsonObjectRequest(Method.GET,url,null,com.android.volley.Response.Listener {
            ...
            try{
                ...
                Picasso.get().load("http$conditionIcon").into(ivTemp)
                if(isday==1){
                    //morning time
                    Picasso.get().load("https://cdn.wallpapersafari.com/76/14/T6ndPX.jpg").into(imgBackground)
                }else{
                    //night time
                    Picasso.get().load("https://images.unsplash.com/photo-1472552944129-b035e9ea3744?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxleHBsb3JlLWZlZWR8M3x8fGVufDB8fHx8&w=1000&q=80").into((imgBackground))
                }
                val forecastObject:JSONObject=it.getJSONObject("forecast")
                val forecast:JSONObject=forecastObject.getJSONArray("forecastday").getJSONObject(0)
